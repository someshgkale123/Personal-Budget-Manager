...
export class JuneComponent implements OnInit {
  ...
  constructor(public authservice:AuthService, public fireservice:AngularFirestore, private router:Router, public june:JuneService) { }
  ...
  ngOnInit(): void {
      ...
      this.june.getbudget().subscribe(data=>
        {
  ...
  createrecord()
  {
    ...
    this.june.createbudget(Value).then(res=>{
      this.Title="";
  ...
  updatebudget(updatedata)
  {
    ...
    this.june.update_data(updatedata.id,data);
  ...
  {
    this.june.deletebudget(budget_id);
    location.reload();
  }
