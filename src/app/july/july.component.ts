...
export class JulyComponent implements OnInit {
  ...
  constructor(public authservice:AuthService, public fireservice:AngularFirestore, private router:Router, public july:JulyService) { }
  ...
  ngOnInit(): void {
      ...
      this.july.getbudget().subscribe(data=>
        {
  ...
  createrecord()
  {
    ...
    this.july.createbudget(Value).then(res=>{
      this.Title="";
  ...
  updatebudget(updatedata)
  {
    ...
    this.july.update_data(updatedata.id,data);
  ...
  {
    this.july.deletebudget(budget_id);
    location.reload();
  }
