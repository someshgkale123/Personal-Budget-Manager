...
export class AugustComponent implements OnInit {
  ...
  constructor(public authservice:AuthService, public fireservice:AngularFirestore, private router:Router, public aug:AugustService) { }
  ...
  ngOnInit(): void {
      ...
      this.aug.getbudget().subscribe(data=>
        {
  ...
  createrecord()
  {
    ...
    this.aug.createbudget(Value).then(res=>{
      this.Title="";
  ...
  updatebudget(updatedata)
  {
    ...
    this.aug.update_data(updatedata.id,data);
  ...
  {
    this.aug.deletebudget(budget_id);
    location.reload();
  }
