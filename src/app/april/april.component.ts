...
export class AprilComponent implements OnInit {
  ...
  constructor(public authservice:AuthService, public fireservice:AngularFirestore, private router:Router, public april:AprilService) { }
  ...
  ngOnInit(): void {
      ...
      this.april.getbudget().subscribe(data=>
        {
  ...
  createrecord()
  {
    ...
    this.april.createbudget(Value).then(res=>{
      this.Title="";
  ...
  updatebudget(updatedata)
  {
    ...
    this.april.update_data(updatedata.id,data);
  ...
  {
    this.april.deletebudget(budget_id);
    location.reload();
  }
