...
export class DecemberComponent implements OnInit {
  ...
  constructor(public authservice:AuthService, public fireservice:AngularFirestore, private router:Router, public dec:DecemberService) { }
  ...
  ngOnInit(): void {
      ...
      this.dec.getbudget().subscribe(data=>
        {
  ...
  createrecord()
  {
    ...
    this.dec.createbudget(Value).then(res=>{
      this.Title="";
  ...
  updatebudget(updatedata)
  {
    ...
    this.dec.update_data(updatedata.id,data);
  ...
  {
    this.dec.deletebudget(budget_id);
    setTimeout(() => {
      location.reload();
    }, 100);
  }
