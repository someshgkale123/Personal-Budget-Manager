...
export class MayComponent implements OnInit {
  ...
  constructor(public authservice:AuthService, public fireservice:AngularFirestore, private router:Router, public may:MayService) { }
  ...
  ngOnInit(): void {
      ...
      this.may.getbudget().subscribe(data=>
        {
  ...
  createrecord()
  {
    ...
    this.may.createbudget(Value).then(res=>{
      this.Title="";
  ...
  updatebudget(updatedata)
  {
    ...
    this.may.update_data(updatedata.id,data);
