...
export class FebruaryComponent implements OnInit {
  ...
  constructor(public authservice:AuthService, public fireservice:AngularFirestore, private router:Router, public feb:FebService) { }
  ...
  ngOnInit(): void {
      ...
      this.feb.getbudget().subscribe(data=>
        {
  ...
  createrecord()
  {
    ...
    this.feb.createbudget(Value).then(res=>{
      this.Title="";
  ...
  updatebudget(updatedata)
  {
    ...
    this.feb.update_data(updatedata.id,data);
  ...
  {
    this.feb.deletebudget(budget_id);
    location.reload();
  }
