...
export class MarchComponent implements OnInit {
  ...
  constructor(public authservice:AuthService, public fireservice:AngularFirestore, private router:Router, public mar:MarService) { }
  ...
  ngOnInit(): void {
      ...
      this.mar.getbudget().subscribe(data=>
        {
  ...
  createrecord()
  {
    ...
    this.mar.createbudget(Value).then(res=>{
      this.Title="";
  ...
  updatebudget(updatedata)
  {
    ...
    this.mar.update_data(updatedata.id,data);
  ...
  {
    this.mar.deletebudget(budget_id);
    location.reload();
  }
